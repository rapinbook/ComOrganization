...
typedef struct CPU_State_Struct {
	...
	uint32_t PIPE_SWITCH[PIPE_STAGE];
	...
	uint32_t EX_MEM_JUMPTAKEN;
	//uint32_t EX_MEM_3rd_INST; // for lw
	...
	uint32_t BRANCH_PRINT;
	uint32_t EACH_STALL;
	uint32_t TOTAL_STALL;
	uint32_t FLAG_JUMP;
	uint32_t FLAG_JR;
	uint32_t FLAG_J;
	uint32_t FLAG_JAL;
